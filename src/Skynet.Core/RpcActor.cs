...
public abstract class RpcActor<TContract> : Actor where TContract : class
{
	...
	protected RpcActor()
	{
		if (this is not TContract)
		{
			throw new InvalidOperationException(
				$"{GetType().FullName} must implement {typeof(TContract).FullName} to be used with RpcActor.");
		}
	...
	protected override async Task<object?> ReceiveAsync(MessageEnvelope envelope, CancellationToken cancellationToken)
	{
		try
		{
			return await _dispatcher.DispatchAsync((TContract)(object)this, envelope, cancellationToken)
	...
	protected virtual Task<object?> OnUnhandledMessageAsync(MessageEnvelope envelope,
		CancellationToken cancellationToken)
	{
		throw new InvalidOperationException(
			$"Unhandled payload '{envelope.Payload?.GetType().FullName}' for actor {GetType().FullName}.");
	}
}
