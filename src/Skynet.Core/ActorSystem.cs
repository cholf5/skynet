...
public sealed class ActorSystem : IAsyncDisposable
{
	...
	/// </summary>
	public async Task<ActorRef> GetOrCreateUniqueAsync<TActor>(string name, Func<TActor> factory, CancellationToken cancellationToken = default) where TActor : Actor
	{
		ArgumentException.ThrowIfNullOrEmpty(name);
		ArgumentNullException.ThrowIfNull(factory);
