...
public sealed class StaticClusterConfiguration
{
	...
	/// </summary>
	public IReadOnlyCollection<StaticClusterNodeConfiguration> Nodes { get; init; } = [];
}
...
public sealed class StaticClusterRegistry : IClusterRegistry
{
	...
	public StaticClusterRegistry(StaticClusterConfiguration configuration, string localNodeId)
	{
		...
		{
			throw new ArgumentException($"Local node '{localNodeId}' is not part of the configuration.",
				nameof(localNodeId));
		}
	}
		...
		lock (_sync)
		{
			if (_serviceToHandle.TryGetValue(name, out var configured) && configured != handle.Value)
			{
				throw new InvalidOperationException(
					$"Service '{name}' is configured with handle {configured} but attempted to register with {handle.Value}.");
			}

			_serviceToHandle[name] = handle.Value;
			_handleToNode[handle.Value] = LocalNodeId;
		}
	}
