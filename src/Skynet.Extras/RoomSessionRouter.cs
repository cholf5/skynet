...
public sealed class RoomSessionRouter : ISessionMessageRouter
{
	...
	public RoomSessionRouter(RoomManager manager, string defaultRoom = "lobby",
		Func<SessionContext, string>? welcomeFormatter = null)
	{
	...
	public async Task OnSessionStartedAsync(SessionContext context, CancellationToken cancellationToken)
	{
		...
		await context
			.SendAsync(_welcomeFormatter?.Invoke(context) ?? $"WELCOME {alias} room={_defaultRoom}", cancellationToken)
			.ConfigureAwait(false);
		if (joinResult.Added)
		{
			await BroadcastSystemAsync(context, _defaultRoom, $"{alias} joined", cancellationToken)
				.ConfigureAwait(false);
		}
	}
	...
	public async Task OnSessionClosedAsync(SessionContext context, SessionCloseReason reason, string? description,
		CancellationToken cancellationToken)
	{
		var alias = EnsureAlias(context);
		var rooms = EnsureMembership(context);
	...
	private async Task HandleJoinAsync(SessionContext context, string[] parts, CancellationToken cancellationToken)
	{
		...
		await context.SendAsync($"JOINED {room} members={_manager.GetMembers(room).Count}", cancellationToken)
			.ConfigureAwait(false);
		if (result.Added)
		{
	...
	private async Task HandleLeaveAsync(SessionContext context, string[] parts, CancellationToken cancellationToken)
	{
		...
		await BroadcastSystemAsync(context, room, $"{EnsureAlias(context)} left", cancellationToken)
			.ConfigureAwait(false);
	}
	...
	private Task HandleWhoAsync(SessionContext context, string[] parts, CancellationToken cancellationToken)
	{
		...
		var payload = members.Count == 0
			? $"WHO {room} none"
			: $"WHO {room} {string.Join(',', members.Select(m => m.SessionId))}";
		return context.SendAsync(payload, cancellationToken).AsTask();
	}
	...
	private async Task BroadcastSystemAsync(SessionContext context, string room, string message,
		CancellationToken cancellationToken)
	{
