...
本仓库是**基础框架**，只收"有唯一合理解"的基础设施（详见
[ADR-0002](adr/0002-framework-scope-and-ecosystem-boundary.md)）：数据库适配、登录体系、
匹配、品类设施（MOBA/MMORPG/FPS 共性组件）等属于生态层，不进本仓库；生态项目
（如基于 Skynet 的游戏服务端重写）以 git submodule + 工程引用方式消费框架。

`Skynet.Extras` 是**可选参考实现**：它演示各插件接缝（指标导出、快照存储、调试控制台、
房间路由）的标准用法，但不是框架的一部分——完全可以不引入 Extras 而自行实现同等能力，
它的变更节奏独立于 Core 的稳定性承诺。
